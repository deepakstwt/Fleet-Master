        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top) {
                    // Trip status icon with animated pulse effect
                    ZStack {
                        Circle()
                            ...
                            .overlay(
                                Circle()
                                    .stroke(statusColor(for: trip.status).opacity(0.5), lineWidth: 2)
                                    .scaleEffect(trip.status == .inProgress ? 1.2 : 1.0)
                                    .opacity(trip.status == .inProgress ? 0.6 : 0.0)
                            )
                    ...
                    // Status badge with shadow and animation
                    Text(trip.status.rawValue)
                        ...
                        .background(
                            Capsule()
                                .fill(statusColor(for: trip.status).opacity(0.15))
                                .shadow(color: statusColor(for: trip.status).opacity(0.3), radius: 3, x: 0, y: 2)
                        )
                        .foregroundColor(statusColor(for: trip.status))
                        .overlay(
                            Capsule()
